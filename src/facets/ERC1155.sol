...
contract ERC1155 is IERC1155 {
        ...
        override
    {
        ...
        require(
            _from == msg.sender || s.operatorApprovals[_from][msg.sender], "ERC1155: Not approved to transfer"
        );
    ...
    function balanceOf(address _owner, uint256 _id) external view override returns (uint256 balance_) {
        balance_ = s.balances[_id][_owner];
    }
        ...
        returns (uint256[] memory balances_)
    {
        ...
        for (uint256 i; i < _owners.length; i++) {
            balances_[i] = s.balances[_ids[i]][_owners[i]];
        }
    }
